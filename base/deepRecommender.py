...
class DeepRecommender(IterativeRecommender):
    ...
    def initModel(self):
        ...
        self.user_embeddings = tf.Variable(tf.compat.v1.truncated_normal(shape=[self.num_users, self.emb_size], stddev=0.005), name='U')
        self.item_embeddings = tf.Variable(tf.compat.v1.truncated_normal(shape=[self.num_items, self.emb_size], stddev=0.005), name='V')
